...
- `lib/core/config/api_environment.dart` 已填测试环境地址（`http://8.220.190.152/whole/`，IP + 明文 HTTP），
  还缺生产地址；生产必须是 HTTPS。
- 加签覆盖的参数必须与实际下发的参数**完全一致**（后端按收到的参数重算签名，多签未下发的字段会 400），
  新增公共参数时同步 `CommonParams.create` 与 `signable`。
